...
import com.alpaca.hyperpong.data.worker.CreateRemoteRankingWorker
import com.alpaca.hyperpong.data.worker.SyncRankingWorker
...
import com.alpaca.hyperpong.domain.model.mappers.asExternalModel
...
import com.alpaca.hyperpong.util.Constants.UNIQUE_WORK_NAME_CREATE_REMOTE_RANKING
import com.alpaca.hyperpong.util.Constants.UNIQUE_WORK_NAME_SYNC_REMOTE_RANKING
import com.alpaca.hyperpong.util.Constants.WORK_DATA_ADMIN_PASSWORD
import com.alpaca.hyperpong.util.Constants.WORK_DATA_IS_ADMIN
...
class Repository @Inject constructor(
    ...
    fun getRanking(id: Long): Flow<Ranking?> = local.getRanking(id = id).map {
        it?.asExternalModel()
    }
    ...
    fun getAllRankings(): Flow<List<Ranking>> = local.getAllRankings().map {
        it.map { entity ->
    ...
    suspend fun searchRanking(name: String, id: Long): Long {
        val ranking = remote.getRanking(
            name = name,
            id = id
        )
    ...
    fun scheduleRemoteRankingCreation(ranking: CreateRankingDTO): Flow<WorkInfo> {
        val data = workDataOf(
            WORK_DATA_RANKING_NAME to ranking.name,
        ...
        val createRemoteRankingRequest = OneTimeWorkRequestBuilder<CreateRemoteRankingWorker>()
            .setInputData(data)
            .setConstraints(constraints)
            .build()
        val workName = "${UNIQUE_WORK_NAME_CREATE_REMOTE_RANKING}_" +
    ...
    fun scheduleSyncRanking(ranking: Ranking): Flow<WorkInfo> {
        ...
        val syncRequest = OneTimeWorkRequestBuilder<SyncRankingWorker>()
            .setInputData(data)
            .setConstraints(constraints)
            .build()
        val workName = "${UNIQUE_WORK_NAME_SYNC_REMOTE_RANKING}_" +
